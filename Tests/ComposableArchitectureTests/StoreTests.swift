...
final class StoreTests: XCTestCase {
  ...
  func testScopedStoreReceivesUpdatesFromParent() {
    ...
    var values: [String] = []
    childStore.state.asPublisher().assertNoFailure()
  ...
  func testParentStoreReceivesUpdatesFromChild() {
    ...
    var values: [Int] = []
    parentStore.state.asPublisher().assertNoFailure()
      .sink(receiveValue: { values.append($0) })
      .store(in: &self.cancellables)
  ...
  func testIfLetTwo() {
    ...
    parentStore
      .ifLet(then: { childStore in
        ...
        vs
          .publisher.asPublisher().assertNoFailure()
          .sink { _ in }
          .store(in: &self.cancellables)
