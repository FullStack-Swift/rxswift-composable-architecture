...
extension EffectPublisher {
  ...
  public var upstream: Observable<Action> {
    self.publisher
  }
}
...
extension Store {
  ...
  public func publisherScope<P: ObservableType, ChildState, ChildAction>(
    state toChildState: @escaping (Observable<State>) -> P,
    action fromChildAction: @escaping (ChildAction) -> Action
  ) -> Observable<Store<ChildState, ChildAction>>
  where P.Element == ChildState {

    func extractChildState(_ state: State) -> ChildState? {
      var childState: ChildState?
      _ = toChildState(Observable.just(state).eraseToAnyPublisher())
        .sink { childState = $0 }
      return childState
    }
    ...
    return toChildState(self.state.eraseToAnyPublisher())
      .map { childState in
        let childStore: Store<ChildState, ChildAction> = Store<ChildState, ChildAction>(
        ...
        childStore.parentCancellable = self.state
          .sink { [weak childStore] state in
            ...
            childStore.state.accept(extractChildState(state) ?? childStore.state.value)
          }
        return childStore
      }
      .eraseToAnyPublisher()
  ...
  public func publisherScope<P: ObservableType, ChildState>(
    state toChildState: @escaping (Observable<State>) -> P
  ) -> Observable<Store<ChildState, Action>>
  where P.Element == ChildState {
